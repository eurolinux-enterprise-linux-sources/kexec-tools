...
class moduleClass(Module):
	...
	def createScreen(self, doDebug = None):
		...
		# roundup availMem because usually /proc/meminfo report less than actual physical memsize
		if (self.availMem + (memRoundup - self.availMem % memRoundup)) < self.lowerThresh and not self.kdumpEnabled:
		...
		self.AdvWindow.set_size_request(500, -1)
		...
		table.attach(self.AdvWindow, 0, 2, 6, 100, gtk.FILL, gtk.FILL|gtk.EXPAND, 5, 5)
		...
		self.vbox.pack_start(internalVBox, True, 15)
	...
	def initializeUI(self):
		if not self.configurable:
			...
			self.showErrorMessage(_("Insufficient memory to auto-enable kdump.  Use system-config-kdump to configure manually"))
